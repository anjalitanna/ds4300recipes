...
def load_neo4j(collection, edge_url, uri='bolt://localhost:7687', user='neo4j', pw='neo4jANJ'):
...
def run_query(query_dct, uri='bolt://localhost:7687', user='neo4j', pw='neo4jANJ'):
...
def get_recipes(uri='bolt://localhost:7687', user='neo4j', pw='neo4jANJ'):
    driver = GraphDatabase.driver(uri, auth=(user, pw))
    with driver.session() as session:
...
def get_ingredients(uri='bolt://localhost:7687', user='neo4j', pw='neo4jANJ'):
    driver = GraphDatabase.driver(uri, auth=(user, pw))
    with driver.session() as session:
...
''''
def get_similar_recipes(name, uri='bolt://localhost:7687', user='neo4j', pw='neo4jANJ'):
    driver = GraphDatabase.driver(uri, auth=(user, pw))
    with driver.session() as session:
...
def get_similar_recipes(name, uri='bolt://localhost:7687', user='neo4j', pw='neo4jANJ'):
    ...
    with driver.session() as session:
        query = "MATCH (n:recipes {name: '" + name + \
                "'})-[r:SIMILAR]-(m:recipes) WITH m, r.similarity as similarity ORDER BY similarity ASC RETURN DISTINCT m LIMIT 5"
        result = session.execute_read(match_recipe, query)
        dct = [list(x.data().values())[0] for x in result]
